...
**大原則: ユーザ確認を要するのは「一方通行（後から変えるのが高コスト／不可逆）な決定」だけ。** 引き返せる決定は Claude が判断して進め、分類の根拠を progress の「決めたこと」（または spec 該当章）に残す。spec / requirement を1本まるごとレビューに乗せない —— その中の一方通行決定だけを抽出して提示する。迷ったら**一方通行側に倒して聞く**（誤って通した損失 > 過剰に聞く損失）。これにより vibe coding の速度を保ちつつ、本当に重要な決定だけにレビューを集約する。

### 一方通行 = ユーザ確認が要る（代表例）

- 外部と握る契約: API のエンドポイント形状・req/res スキーマ、保存・通信フォーマット、外部サービス連携の取り決め
- セキュリティ / ポリシーのスコープ: 認証・権限の方針、fail-closed の対象、課金まわり
- 後から移行コストの高い永続化スキーマ・データモデル
- 不可逆なプロダクト判断: MVP で何を作る/作らない、公開・課金に関わる確定
- マイルストン境界の GO/NO-GO（特にアーキ確定 = スタック表の pin）

### 引き返せる = 通過（Claude 判断 + progress にログ。代表例）

- 内部実装構造、命名、ファイル分割、リファクタ
- チューニング値・暫定パラメータ（後で差し替え可能なもの）
- スタック表に載っている範囲でのライブラリの内部利用方法
- UI の細部・文言（後から変えられるもの）

### 提示のしかた

一方通行決定を出すときは **「見てほしい点」「なぜ一方通行か」「目安時間」** を添える（`05-PROGRESS.md` の「レビュー待ち」、rn 形式）。引き返せる決定はレビューに乗せず、根拠だけ progress に残す。

## `🔍 reviewed` の定義

spec / requirement 全体の通読承認ではなく **「それが含む一方通行決定をユーザが確認済」** を意味する。

- 一方通行決定があれば、ユーザ確認 OK をもらってから `🔍 reviewed` を付ける。確認した日付とコメント要旨を冒頭に1行追記する。
- 一方通行決定が無ければ Claude が自己判定で進め、「一方通行決定なし・Claude 判定」の旨を progress に残す（`🔍 reviewed` 化のためにユーザへ確認させない）。
- Claude 単独で一方通行決定を承認済扱いにしない（自作自演になる）。
...
- 実装に入る前に `docs/08-CODING.md`（コーディング規則）を読む。スタイルは lint / formatter / typecheck に委譲し、判断を要する規則だけがこの doc にある。
...
- 変更のリスクに見合う検証を行う。
- 大きな機能でも、spec に含む一方通行決定だけユーザ確認してから実装する（引き返せる部分は spec の `🔍 reviewed` を待たず先行実装してよい）。
...
- skill が走らせる git / シェルコマンドは隠さず見える形で実行する。

## ドキュメント分割

- `05-PROGRESS.md` は現在地サマリに徹する。詳細ログは `progress/` に置く。
- ideas は「何を作るか」の壁打ちだけに使う。作ると決めた後の具体化は `app-design/` に移す。
- `app-design/` は考え中の作業場。固まった内容は `01-PRODUCT.md`（プロダクト）/ `04-ARCHITECTURE.md`（技術）に反映し、二重管理しない。app-design に入ったら最初に `technical.md`「プラットフォーム & 言語」（アプリ種別 → 言語）を一方通行決定として確定する。
