...
| 環境起因で失敗した・必要だった環境情報を得た | `docs/09-ENVIRONMENT.md` |
...
- ideas は「何を作るか」だけ。決めた後の具体化は `app-design/` へ。app-design に入ったら最初に `technical.md`「プラットフォーム & 言語」（種別 → 言語）を一方通行決定として確定する。
