# Switches（切り替え管理）

実験で比較・切り戻したい変種の台帳。**動いていた状態を壊さない**ための可逆性の要。

## 記録方法

- 変種を作ったらここに登録する（ID・対象・場所 / 切替方法・status）。
- 機構は問わない —— config / フラグでパラメータ化でも、`mvp/backend/<variant>/` 等に並置でもよい。**切替えと比較ができればよい**。
- 結果は `experiments.md` に。不要になった変種は status を `retired` にする（消す前に retired で残し、必要なら戻せるように）。

## 変種一覧

| ID | 対象（プロンプト / スキーマ / 検索 等） | 場所 / 切替方法 | status | メモ |
|---|---|---|---|---|
| なし | | | | |

status: `active`（現用） / `candidate`（比較中） / `retired`（不要・参照用に残置）
