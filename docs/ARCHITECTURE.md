...
スタックの実体は [../scaffold/](../scaffold/README.md)（mvp/・app/ はそのコピーで開始）。具体バージョンの正は [../scaffold/package.json](../scaffold/package.json)（lockfile で固定）。
...
- scaffold 由来のレーン（mvp/・app/）では `npm run verify` が上記を一括で回す（scripts の正は [../scaffold/package.json](../scaffold/package.json)）。
