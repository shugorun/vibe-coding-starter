...
pin = ユーザ承認時に該当行の status を `🔍 reviewed` にする（**P3 ゲート**で本実装のスタックを確定）。**この表は本実装(P5)のスタック**であり、P2 MVP の使い捨てプロトタイプの技術選定は載せない（MVP のスタックは progress / `mvp/README.md` に軽く記録するだけで、本実装には引き継がない —— 例: 本番モバイルでも MVP はウェブで試した、等）。
