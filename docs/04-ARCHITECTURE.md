...
pin = ユーザ承認時に該当行の status を `🔍 reviewed` にする。**pin は 2 段階**:

- **P1 ゲート（MVP の前）**: アプリ種別・言語 / ランタイムを pin。MVP を作るのに必須なため。
- **P3 ゲート（MVP の後）**: 残りの行を MVP の学びを踏まえて pin。

| 領域 | 採用 | status | ADR | 根拠 |
|---|---|---|---|---|
| アプリ種別 | 未設定 | 📝 draft | | P1 で pin |
| 言語 / ランタイム | 未設定 | 📝 draft | | P1 で pin |
