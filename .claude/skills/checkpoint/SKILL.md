...
作業単位の区切りで「progress 追記 → 検証 → 緑なら commit」を行う。粒度は **コミット1個分の動く増分**。チャット毎（細かすぎ）でもセッション末まとめ（粗すぎ）でもない。**push はここではしない**（リモート同期はセッション末の `/wrapup-project` でまとめる）。
...
- 1 フェーズ / 1 機能スライスが動いた
- 非自明な技術・プロダクト判断をした
- ブロッカー・既知不具合を踏んだ（その場合は `docs/issues/open/` に issue も起票）
...
`docs/progress/<week>.md`（無ければ作成。週番号は `"{0}-W{1:D2}" -f (Get-Date).Year, [int](Get-Date -UFormat %V)`）に **1 作業単位 = 1 エントリ**で追記する。1日分をまとめて1エントリに詰め込まない（読み返せなくなる）。
...
プロジェクトで使える検証を走らせる（存在するものだけ: 型チェック / lint / unit / build / smoke など）。すべて通れば「緑」。検証が1つも無いプロジェクトは「動作確認した」を緑とみなしてよい。
...
commit メッセージは作業種別の prefix を付ける（`docs/02-GUIDELINES.md` の表と同じ）:
...
- curated doc（`01-PRODUCT` / `04-ARCHITECTURE` / `app-design` / `requirements` / `specs`）は checkpoint では基本触らない。マイルストン級の変化は別途、または `/wrapup-project` で反映する。
